...
## 相関係数
```{r}
correlation <- function(human_df, df, vehicles) {
    statistics <- c()
    p_values <- c()
    estimates <- c()
    for (vehicle in vehicles) {
        human_scores <- unlist(human_df[vehicle, ])
        scores <- c()
        ...
        corr <- cor.test(scores, human_scores, method = "spearman")
...
det_object_corr <- correlation(human_df, det_object_df, vehicles=rownames(human_df))
smx_object_corr <- correlation(human_df, smx_object_df, vehicles=rownames(human_df))
```
#### Deterministic
```{r, echo=FALSE}
...
for (key in names(det_triangle_dfs)) {
    ...
    triangle_corr <- correlation(human_df, triangle_df, vehicles)
...
# プロットの保存
ggsave("./fig/corr_all.pdf", plot, width = 10, height = 8)
```
```{r, echo=FALSE}
plot
```
...
# プロットの保存
ggsave("./fig/corr_p_010.pdf", plot, width = 10, height = 8)
```
```{r, echo=FALSE}
plot
```
...
# プロットの保存
ggsave("./fig/corr_p_005.pdf", plot, width = 10, height = 8)
```
```{r, echo=FALSE}
plot
```