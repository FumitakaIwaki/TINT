---
title: "TINTシミュレーション結果の順位相関係数 (Softmax)"
...
object_file <- "../results/object_softmax/object_蝶_踊り子.csv"
triangle_dir <- "../results/triangle_softmax"
...
ggsave("./fig/softmax_rank_corr_all.pdf", plot, width = 10, height = 10)
```
```{r, echo=FALSE}
plot
```
...
ggsave("./fig/softmax_rank_corr_p_010.pdf", plot, width = 10, height = 10)
```
```{r, echo=FALSE}
plot
```
...
ggsave("./fig/softmax_rank_corr_p_005.pdf", plot, width = 10, height = 10)
```
```{r, echo=FALSE}
plot
```