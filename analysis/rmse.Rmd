...
### 全構造考慮
```{r, echo=FALSE}
det_structure_rmse <- calc_rmse(human_df, det_structure_df, vehicles=rownames(human_df))
smx_structure_rmse <- calc_rmse(human_df, smx_structure_df, vehicles=rownames(human_df))
```
#### Deterministic | Softmax
```{r, echo=FALSE}
df <- data.frame(
    vehicle = rownames(human_df),
    deterministic_rmse = det_structure_rmse,
    softmax_rmse = smx_structure_rmse
)
DT::datatable(df)
```
#### Sum of RMSE
...
- 構造無視よりは悪い
- 他手法同様softmaxの方がいい
- deterministicとsoftmaxで有意差出ず
```{r, echo=FALSE}
print(paste("deterministic: ", sum(det_structure_rmse)))
print(paste("softmax: ", sum(smx_structure_rmse)))
print(t.test(det_structure_rmse, smx_structure_rmse, paired = TRUE))
```