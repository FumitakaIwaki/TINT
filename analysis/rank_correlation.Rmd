...
deterministic_structure_file <- "../results/whole_structure_deterministic/whole_蝶_踊り子.csv"
softmax_structure_file <- "../results/whole_structure_softmax/whole_蝶_踊り子.csv"
```
...
### 全構造考慮の結果データ
```{r, echo=FALSE, message=FALSE, warning=FALSE}
det_structure_df <- read_csv(deterministic_structure_file)
smx_structure_df <- read_csv(softmax_structure_file)
```
#### Deterministic
```{r, echo=FALSE}
DT::datatable(det_structure_df)
```
#### Softmax
```{r, echo=FALSE}
DT::datatable(smx_structure_df)
```
...
### 構造無視
```{r, echo=FALSE, warning=FALSE}
det_structure_corr <- rank_correlation(human_df, det_structure_df, vehicles=rownames(human_df))
smx_structure_corr <- rank_correlation(human_df, smx_structure_df, vehicles=rownames(human_df))
```
#### Deterministic
```{r, echo=FALSE}
DT::datatable(det_structure_corr)
```
#### Softmax
```{r, echo=FALSE}
DT::datatable(smx_structure_corr)
```
...
visualize <- function(combined_data, vehicles) {
    ...
    scale_shape_manual( # 点の形の設定
        values = c("構造無視" = 16, "三角構造考慮 (平均)" = 17, "全構造考慮" = 15, "三角構造考慮" = 2)
    ) +
    scale_color_manual( # 点の色の設定
        values = c("Deterministic" = "coral", "Softmax" = "royalblue")
    ) +
    scale_alpha_manual(
        values = c("構造無視" = 1, "三角構造考慮 (平均)" = 1, "全構造考慮" = 1, "三角構造考慮" = 0.5)
    ) +
    scale_fill_manual(
        values = c("Deterministic" = "coral", "Softmax" = "royalblue")
    ) +
    scale_linetype_manual(
        values = c("構造無視" = "solid", "三角構造考慮 (平均)" = "twodash", "全構造考慮" = "dashed", "三角構造考慮" = "blank")
    ) +
...
p_det_structure_corr <- det_structure_corr %>%
    mutate(
        mode = "全構造考慮",
        method = "Deterministic"
    )
p_smx_structure_corr <- smx_structure_corr %>%
    mutate(
        mode = "全構造考慮",
        method = "Softmax"
    )
...
combined_data <- bind_rows(
    ...
    p_det_triangle_corr,
    p_smx_triangle_corr,
    p_det_structure_corr,
    p_smx_structure_corr
)
# プロットの順番を制御
