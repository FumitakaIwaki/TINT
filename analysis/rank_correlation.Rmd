...
# データの結合
combined_data <- bind_rows(object_corr, all_triangle_corr, all_triangle_corr_avg)

# プロットの作成
plot <- ggplot(combined_data, aes(x = vehicle, y = corr, color = source, shape = source)) +
  geom_point(size = 3) +  # 点を描画、色と形をsourceごとに設定
  scale_y_continuous(breaks = seq(-1, 1, by = 0.2)) +  # y軸を0.2刻みで設定
...
ggsave("./fig/rank_corr_all.pdf", plot, width = 10, height = 10)
```
```{r, echo=FALSE}
plot
```
...
# データの結合
combined_data_filtered <- bind_rows(object_corr_filtered, all_triangle_corr_filtered, all_triangle_corr_avg)
...
ggsave("./fig/rank_corr_p_010.pdf", plot, width = 10, height = 10)
```
```{r, echo=FALSE}
plot
```
...
ggsave("./fig/rank_corr_p_005.pdf", plot, width = 10, height = 10)
```
```{r, echo=FALSE}
plot
```